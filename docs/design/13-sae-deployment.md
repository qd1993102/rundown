...
> 固定基于项目根目录解析，不随进程 cwd（release 目录）漂移。
> `get_config()` 自动读取部署环境文件 `/etc/neurun/neurun.env`（发布脚本写入
> `NEURUN_DATA_DIR` / `NEURUN_INVITE_CODES_FILE`），因此 CLI 管理员命令与 Web
...
`code_deploy_application/`。控制台入口只需显式传入 `SOURCE_DIR`；发布脚本直接把该干净 Git 工作树的完整
`HEAD` SHA 作为唯一候选版本。脚本不接受、不校验也不依赖外部 `EXPECTED_COMMIT`；旧控制台入口即使仍传入该
环境变量也不得阻断平台已 checkout 的提交。入口和发布脚本都不得另外固定、推测或 `fetch` 某个远程分支。
脚本仍必须确认暂存区是干净 Git 工作树、`HEAD` 是完整 40 位小写 SHA 且存在 `pyproject.toml`。运行中 release
的实际 SHA 仍必须通过 `/healthz` 回传并与本次平台 checkout 一致；平台是否选中远程最新 revision 由发布任务配置
保证，不由服务器脚本跨分支判定。
